...
app.get("/teacher/grades", isLoggedIn, async function (req, res) {
    ...
    const resp = await Promise.all(currentUser.students.map(async (student) => {
        ...
        return new Promise((resolve) => {
            resolve({ name: studentDoc.name, studentId: studentDoc.studentId, grades: student.grades, attendence: student.attendence });
        });
    }));
    // res.send(resp);
    //console.log("rr = ", resp);
...
app.get("/teacher/attendence", isLoggedIn, async function (req, res) {
    ...
    const resp = await Promise.all(currentUser.students.map(async (student) => {
        ...
        const studentDoc = await Student.findById(studentId);
        return new Promise((resolve) => {
            resolve({ name: studentDoc.name, studentId: studentDoc.studentId,classes: currentUser.count ,grades: student.grades, attendence: student.attendence });
        });
    }));
    // res.send(resp);
    //console.log("rr = ", resp);
